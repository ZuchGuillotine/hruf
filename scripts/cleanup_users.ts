...
async function cleanupUsers() {
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      // First delete related records from tables with foreign key references
      
      // 1. Delete from chat_summaries
      const deletedChatSummaries = await tx.delete(chatSummaries)
      ...
      const deletedQueryChats = await tx.delete(queryChats)
      ...
      const deletedQualitativeLogs = await tx.delete(qualitativeLogs)
      ...
      const deletedSupplementLogs = await tx.delete(supplementLogs)
      ...
      const deletedHealthStats = await tx.delete(healthStats)
      ...
      const deletedSupplements = await tx.delete(supplements)
