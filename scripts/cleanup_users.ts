...
async function cleanupUsers() {
  ...
  try {
    ...
    console.log(
      `Found ${userIdsToKeep.length} users to keep with IDs: ${userIdsToKeep.join(', ')}`
    );
    ...
    const deletedChatSummaries = await db
    ...
    const deletedQueryChats = await db
      .delete(queryChats)
    ...
    const deletedQualitativeLogs = await db
      .delete(qualitativeLogs)
    ...
    const deletedSupplementLogs = await db
    ...
    const deletedHealthStats = await db
      .delete(healthStats)
    ...
    const deletedSupplements = await db
      .delete(supplements)
