...
describe('Embedding Service Tests', () => {
  ...
  const testContent = "Magnesium supplementation has improved my sleep quality and reduced muscle cramps.";
  ...
  test('Should create and store log embedding', async () => {
    ...
    await embeddingService.createLogEmbedding(
      testLogId, 
      testContent,
      'qualitative'
    );
  ...
  test('Should find similar content by semantic meaning', async () => {
    ...
    if (results1.length > 0 && results2.length > 0 && results3.length > 0) {
      ...
      const avgSimilarity1 = results1.reduce((sum, item) => sum + item.similarity, 0) / results1.length;
      const avgSimilarity3 = results3.reduce((sum, item) => sum + item.similarity, 0) / results3.length;
