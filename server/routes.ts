...
export function registerRoutes(app: Express): Server {
  ...
  app.post('/api/chat', requireAuth, async (req, res) => {
    try {
      ...
      const userContext = await constructUserContext(
  ...
  app.get('/api/chat/history', requireAuth, async (req, res) => {
    try {
      const history = await db
        ...
        .where(and(eq(qualitativeLogs.userId, req.user!.id), eq(qualitativeLogs.type, 'chat')))
  ...
  app.post('/api/admin/supplements', requireAuth, requireAdmin, async (req, res) => {
    try {
      const [newSupplement] = await db.insert(supplementReference).values(req.body).returning();
  ...
  app.put('/api/supplements/:id', requireAuth, async (req, res) => {
    try {
      const [updated] = await db
        ...
        .set(req.body)
        .where(
          and(eq(supplements.id, parseInt(req.params.id)), eq(supplements.userId, req.user!.id))
        )
        .returning();
  ...
  app.delete('/api/admin/users/delete-non-admin', requireAuth, requireAdmin, async (req, res) => {
    try {
      const result = await db.delete(users).where(eq(users.isAdmin, false)).returning();
  ...
  app.delete('/api/labs/:id', requireAuth, async (req, res) => {
    try {
      const [result] = await db
        .delete(labResults)
        .where(and(eq(labResults.id, parseInt(req.params.id)), eq(labResults.userId, req.user!.id)))
        .returning();
