...
describe('OpenAI Service Tests', () => {
  ...
  test('Should generate response from OpenAI', async () => {
    ...
    // Test messages
    const messages = [{ role: 'user', content: 'What are the benefits of Vitamin C?' }];
  ...
  test('Should estimate token count correctly', () => {
    ...
    const mediumText =
      'Vitamin D3 is essential for calcium absorption and bone health. It also plays a role in immune function and mood regulation.';
    const longText =
  ...
  test('Should handle model override parameter', async () => {
    ...
    // Test messages
    const messages = [{ role: 'user', content: 'What is a good supplement stack for energy?' }];
