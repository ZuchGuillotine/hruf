...
import { qualitativeLogs, queryChats } from '@core/db';
...
export async function* queryWithAI(
  ...
  try {
    ...
    try {
      ...
      yield {
        error:
          streamError instanceof Error ? streamError.message : 'Error connecting to AI service',
        streaming: false,
      };
    }
  } catch (error) {
    ...
    yield {
      error:
        "I'm having trouble understanding your question right now. Please try again in a moment.",
      streaming: false,
    };
  }
}
