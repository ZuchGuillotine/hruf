...
class SupplementLookupService {
  ...
  async findSupplementLogs(
    ...
    try {
      ...
      const searchTerms = supplementName.split(' ')
  ...
  extractSupplementNames(query: string): string[] {
    ...
    const commonSupplements = [
      'creatine', 'vitamin d', 'vitamin c', 'vitamin b', 'magnesium', 
      'zinc', 'omega-3', 'fish oil', 'protein', 'pre-workout', 
      'bcaa', 'glutamine', 'collagen', 'probiotics', 'melatonin'
    ];
    
    const queryLower = query.toLowerCase().trim();
    return commonSupplements.filter(supplement => 
      queryLower.includes(supplement.toLowerCase())
    );
  }
      ...
      for (const name of supplementNames) {
        ...
        if (logs.length > 0) {
          ...
          logs.forEach(log => {
            ...
            contextContent += `[${logDate}] ${log.supplementName || name}, ` +
