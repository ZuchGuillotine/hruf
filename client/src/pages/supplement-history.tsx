...
export default function SupplementHistory() {
  ...
  useEffect(() => {
    const todayButton = scrollContainerRef.current?.querySelector(
      `button[data-date='${format(today, 'yyyy-MM-dd')}']`
    );
    if (todayButton) {
        ...
        <div className="bg-[#1b4332] rounded-lg p-6">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <Button variant="ghost" onClick={prevMonth} className="text-white hover:bg-white/10">
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <h3 className="text-lg font-semibold text-white">
                {format(currentMonth, 'MMMM yyyy')}
              </h3>
              <Button variant="ghost" onClick={nextMonth} className="text-white hover:bg-white/10">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white/5 rounded-lg p-4">
                    ...
                    {qualitativeLogs.length > 0 ? (
                      ...
                      <p className="text-white/70">No notes or chat history for this date.</p>
                    )}
                  </div>
                </div>
              )}
              ...
              {selectedChat.map((message, index) => (
                <div
                  ...
                  className={`p-3 rounded-lg ${
                    message.role === 'assistant' ? 'bg-green-50 ml-4' : 'bg-blue-50 mr-4'
                  }`}
                >
