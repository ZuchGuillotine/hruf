...
export async function seedSupplements() {
  try {
    ...
    const existingCount = await db.select({ count: sql`count(*)` }).from(supplementReference);
    ...
    for (let i = 0; i < initialSupplements.length; i += batchSize) {
      ...
      await db.insert(supplementReference).values(batch).onConflictDoNothing();

      console.log(`Seeded batch of ${batch.length} supplements`);
    }

    // Verify seeding
    const finalCount = await db.select({ count: sql`count(*)` }).from(supplementReference);
